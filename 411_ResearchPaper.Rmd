...
# 1.1 Ancient History of Cryptography  
### Caesar's Cipher
...
# 2.1 Finding Large Primes(prob of prime 1/4, etc. , )
...
### Theorem 2: Fermat Primality Test  
...
\end{aligned}
$$
ends with $1$. If $a^d$ is not congruent to $1(mod\:n)$, then the value directly preceding the first appearance of $1$ is $n-1$.  
...
Given an odd integer $n$, find an integer $s$ so that $n-1=2^sq$ for some odd $q$. Then,  

1. Pick a random integer $a$ such that $1\leq a \leq n-1$.  
2. If $a^q \equiv 1 (mod\: n)$, then $n$ might be prime and passes the test.  
3. Otherwise, for $i= 0, 1, ..., s-1$, see if $a^{2^iq} \equiv -1 (mod\: n)$. If so, then $n$ could be prime and passes the test.  

4. Otherwise, $n$ is composite.  [@NumberTheoryPrimality]


 
#### Rabin-Miller Test Example:  

Suppose we wish to determine whether 645 is prime. Then,  $n-1= 644 = 2^2 \cdot 161$  
1. Let $a=2$.  
2. $2^{161} \equiv 257 (mod\: 645)$. Since $2^{161}$ is not congruent to $\pm 1(mod\: n)$, we move on to step 3.  
3.
$$
\begin{aligned} 
...
# 2.2 Using Euler's Totient Function 
...
### Theorem 4: Unique Factorization Theorem
